...
Goal_Think::Goal_Think(Raven_Bot* pBot):Goal_Composite<Raven_Bot>(pBot, goal_think)
{
  ...
  double BaitBias = RandInRange(LowRangeOfBias, HighRangeOfBias);
  ...
  m_Evaluators.push_back(new GetBAIT_Evaluator(BaitBias));
}
...
void Goal_Think::AddGoal_Bait()
{
    if (notPresent(goal_bait))
    {
        RemoveAllSubgoals();
        AddSubgoal(new Goal_Bait(m_pOwner));
    }
}
